...
# --- Case 1: managed + unmanaged → keep file, drop only Zolven's blocks -------
t1="$(mktemp -d)"
...
if [ -f "$cfg" ]; then check 0 "case1: config file preserved (unmanaged account present)"; else check 1 "case1: config file preserved (unmanaged account present)"; fi
if grep -q 'Zolven mail account' "$cfg"; then check 1 "case1: no Zolven markers remain"; else check 0 "case1: no Zolven markers remain"; fi
...
# --- Case 2: only Zolven-managed accounts → remove the file entirely ----------
t2="$(mktemp -d)"
...
run_stage "$t2"
if [ -f "$t2/.config/himalaya/config.toml" ]; then check 1 "case2: Zolven-only config file removed"; else check 0 "case2: Zolven-only config file removed"; fi
...
#     → keep the file (non-whitespace survivor rule), strip Zolven's markers.
t5="$(mktemp -d)"
...
if grep -q 'Zolven mail account' "$cfg5"; then check 1 "case5: no Zolven markers remain"; else check 0 "case5: no Zolven markers remain"; fi
